...
function Icon({ d, viewBox = '0 0 24 24' }: { d: string; viewBox?: string }) {
  return (
    <svg width="16" height="16" viewBox={viewBox} fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
      <path d={d} />
    </svg>
  );
}
      ...
      <nav className="nav" aria-label="Primary">
        ...
        <div className="nav-items">
          {NAV.map((item) => {
            ...
            const count = item.href === '/inbox' ? openInbox : item.href === '/' && failures > 0 ? failures : 0;
            return (
              <Link key={item.href} href={item.href} className={`nav-item ${active ? 'active' : ''}`}>
                <Icon d={item.d} />
                {item.label}
                {count > 0 && <span className={`count ${item.href === '/' ? 'alert' : ''}`}>{count}</span>}
              </Link>
            );
          })}
        </div>
